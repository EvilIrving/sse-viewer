# SSE Inspector Chrome 扩展项目说明

## 项目概述
SSE Inspector 是一个 Chrome DevTools 扩展，用于捕获和可视化网页中的 Server-Sent Events (SSE) 流数据。支持原生 EventSource API、基于 fetch 的 text/event-stream 请求，以及 `@microsoft/fetch-event-source` 等第三方 SSE 库。
...
**流程**:
1. 调用 `chrome.devtools.panels.create` 创建 "SSE Inspector" 面板
   - 第一个参数：面板名称
...
**最后更新**: 2026-01-12  
**当前状态**: 1.0.0 正式版发布，支持主流 SSE 实现方式，提供类似 Network 面板的交互体验，采用 streamId 机制实现精准连接区分，**已实现完整的断线重连机制**，**已实现国际化支持（中英日法四种语言，自动适配浏览器语言）**，**DevTools 面板图标已适配深色/浅色主题**。
...
setInterval(() => {
  console.log('[SSE Inspector] Keepalive ping');
}, KEEPALIVE_INTERVAL);
```
